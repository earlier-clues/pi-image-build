...
#   --cert-path PATH         path to TLS CA certificate (PEM format)
#
...
#   install_mqtt_broker --cert-path /etc/mosquitto/ca.crt
install_mqtt_broker() {
    local port="1883"
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --cert-path)
                [[ -n "${2:-}" ]] || { echo "install_mqtt_broker: --cert-path needs an argument" >&2; return 2; }
                cert_path="$2"; shift 2 ;;
            *)
    ...
    local tls_block=""
    if [[ -n "$cert_path" ]]; then
        cert_path_esc=$(sed_escape "$cert_path")
        tls_block="cafile ${cert_path_esc}"
    fi
    ...
    local port_esc auth_block_esc tls_block_esc
    port_esc=$(sed_escape "$port")
    auth_block_esc=$(sed_escape "$auth_block")
    tls_block_esc=$(sed_escape "$tls_block")

    sed -i \
        -e "s|@@PORT@@|${port_esc}|g" \
        -e "s|@@AUTH_BLOCK@@|${auth_block_esc}|g" \
        -e "s|@@TLS_BLOCK@@|${tls_block_esc}|g" \
        "$dst"
