...
| `tailscale` | `TAILSCALE_AUTHKEY` | `TAILSCALE_HOSTNAME=$HOSTNAME`, `TAILSCALE_FLAGS=` | Install Tailscale, enroll on first boot, self-disable. Default: SSH via OpenSSH + baked pubkey. Set `TAILSCALE_FLAGS=--ssh` for team-shared images — lets tailnet members SSH in by identity, no pubkey distribution needed (requires `ssh:` ACL). |
