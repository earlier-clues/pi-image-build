...
usage() {
    awk '/^# Usage:/,/^# Platform:/ {print}' "$0" | sed 's/^# \{0,1\}//'
}
...
# Initialize variables for new-contract dispatch (may be populated next).
RUN_MODULES_SH=""
...
if [[ "$PAYLOAD_CONTRACT" == "modules" ]]; then
    ...
    parsed_list="$(parse_modules_list "$PAYLOAD_DIR/modules.list")"
    MODULE_NAMES=()
